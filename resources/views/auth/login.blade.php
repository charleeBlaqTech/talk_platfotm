...
<div class="w-[30%] py-6 px-3 bg-white rounded-xl">
    @if(session()->has('error'))
        <p class=" w-[100%] text-center my-8 bg-red-400 border-2 text-sm text-red-600">{{session('error')}}</p>
    @endif
    <h1 class="w-[100%] text-center text-3xl my-3">Sign In</h1>
    <form action="{{ route('login.store') }}" method="post" class="flex flex-col align-middle gap-3 w-full">
        @csrf
